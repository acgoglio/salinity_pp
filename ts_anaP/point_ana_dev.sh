...
if [[ $EXTRACTION_FLAG == 1 ]]; then
  ...
  if [[ -d $ANA_WORKDIR ]]; then
     ...
     cp ${SRC_DIR}/point_ana.ini ${ANA_WORKDIR}/point_ana.ini_$(date +%Y%m%d_%H%M%S)
 ...
 if [[ $MYDIAG_FLAG == 0 ]]; then
  ...
  if [[ -d $ANA_WORKDIR ]]; then
     cd $ANA_WORKDIR
     echo "WORKDIR: $ANA_WORKDIR"
     cp ${SRC_DIR}/point_ana.ini ${ANA_WORKDIR}
  fi
fi
  ...
  while [[ $IDX_IN -lt $INSET_NUM ]]; do
    ...
    for VAR in ${DMMM_FIELDS[@]}; do 
      ...
      for DMMEXT in $EXT_INS; do   
        ...
        #echo "cdo -infon -sellonlatbox,-18.125,-10.000,30.1875,45.97917 $DMMEXT | grep -v "Date" | grep "$VAR"  >> $DMMM_OUTFILE" 
        #cdo -infon -sellonlatbox,-18.125,-10.000,30.1875,45.97917 $DMMEXT | grep -v "Date" | grep "$VAR"  >> $DMMM_OUTFILE
        # Quasi Med cut:
        echo "cdo -infon -sellonlatbox,0.000,36.29167,30.1875,45.97917 $DMMEXT | grep -v "Date" | grep "$VAR"  >> $DMMM_OUTFILE" 
        cdo -infon -sellonlatbox,0.000,36.29167,30.1875,45.97917 $DMMEXT | grep -v "Date" | grep "$VAR"  >> $DMMM_OUTFILE
        rm -v $DMMEXT
      done
 ...
 if [[ $MYDIAG_FLAG == 1 ]]; then
    ...
    for MYDIAG_FIELD in ${MYDIAG_SHORT_NAMES[@]}; do
        ...
        if [[ -f $MYDIAG_FILE_1 ]] && [[ -f $MYDIAG_FILE_2 ]] ; then
          ...
          GNUPLOT_MYDIAG_TMP="mydiag_tmp.gpl_${MYDIAG_FIELD}"
          ...
          echo "set term jpeg giant size 1300,800 font \"Times,45\"" >> ${GNUPLOT_MYDIAG_TMP}
          ...
          echo "plot '$MYDIAG_PLOTINFILE_1' using 1:3 with line lw 2 lt rgb \"red\" title gprintf(\"${MYDIAG_INTAG_1} (AVG = %g ${MYDIAG_PLOTUDM} ) \", STATS1_mean), '$MYDIAG_PLOTINFILE_2' using 1:3 with line lw 2 lt rgb \"black\" title gprintf(\"${MYDIAG_INTAG_2} (AVG = %g ${MYDIAG_PLOTUDM} )\", STATS2_mean) " >> ${GNUPLOT_MYDIAG_TMP} 
    ...
    for MYDIAG_FIELD in ${MYDIAG_SHORT_NAMES[@]}; do
        ...
        if [[ -f $MYDIAG_FILE_1 ]] && [[ -f $MYDIAG_FILE_2 ]] && [[ -f $MYDIAG_FILE_3 ]] ; then
          ...
          GNUPLOT_MYDIAG_TMP="mydiag_tmp.gpl_$MYDIAG_FIELD"
          echo "#" > ${GNUPLOT_MYDIAG_TMP}
          echo "set term jpeg size 1200,700 font giant" >> ${GNUPLOT_MYDIAG_TMP} #1500,1200
          echo "set output \"$MYDIAG_PLOT\" " >> ${GNUPLOT_MYDIAG_TMP}
          ...
          echo "set key outside" >> ${GNUPLOT_MYDIAG_TMP}
           ...
           echo "plot '$MYDIAG_PLOTINFILE_3' using 1:3 with line lw 2 lt rgb \"red\" title \"${MYDIAG_INTAG_3}\",'$MYDIAG_PLOTINFILE_1' using 1:3 with line lw 2 lt rgb \"orange\" title \"${MYDIAG_INTAG_1}\",'$MYDIAG_PLOTINFILE_2' using 1:3 with line lw 2 lt rgb \"blue\" title \"${MYDIAG_INTAG_2}\"" >> ${GNUPLOT_MYDIAG_TMP}
          ...
          echo "plot '$MYDIAG_PLOTINFILE_DIFF_1' using 1:3 with line lw 2 lt rgb \"orange\" title \"${MYDIAG_INTAG_1} - ${MYDIAG_INTAG_2}\",'$MYDIAG_PLOTINFILE_DIFF_2' using 1:3 with line lw 2 lt rgb \"red\" title \"${MYDIAG_INTAG_3} - ${MYDIAG_INTAG_2}\" " >> ${GNUPLOT_MYDIAG_TMP}
          #echo "plot '$MYDIAG_PLOTINFILE_DIFF_1' using 1:3 with points pt 1 ps 1 lt rgb \"orange\" title \"Diff: ${MYDIAG_INTAG_1} - ${MYDIAG_INTAG_2}\",'$MYDIAG_PLOTINFILE_DIFF_2' using 1:3 with points pt 1 ps 1 lt rgb \"red\" title \"Diff: ${MYDIAG_INTAG_3} - ${MYDIAG_INTAG_2}\", STATSD1_mean lw 2 lc rgb \"black\" title gprintf(\"${MYDIAG_INTAG_1}-${MYDIAG_INTAG_2} mean %g ${MYDIAG_PLOTUDM} \", STATSD1_mean) , STATSD1_lo_quartile lw 2 lc rgb \"gray\" title \"${MYDIAG_INTAG_1}-${MYDIAG_INTAG_2} 1st quartile\",STATSD1_up_quartile lw 2 lc rgb \"gray\" title \"${MYDIAG_INTAG_1}-${MYDIAG_INTAG_2} 3rd quartile\",STATSD2_mean lw 2 lc rgb \"blue\" title gprintf( \"${MYDIAG_INTAG_3}-${MYDIAG_INTAG_2} mean = %g ${MYDIAG_PLOTUDM} \", STATSD2_mean ), STATSD2_lo_quartile lw 2 lc rgb \"red\" title \"${MYDIAG_INTAG_3}-${MYDIAG_INTAG_2} 1st quartile\",STATSD2_up_quartile lw 2 lc rgb \"red\" title \"${MYDIAG_INTAG_3}-${MYDIAG_INTAG_2} 3rd quartile\"  " >> ${GNUPLOT_MYDIAG_TMP}
         echo "plot '$MYDIAG_PLOTINFILE_DIFF_M' using 1:3 with line lw 2 lt rgb \"dark-green\" title \"${MYDIAG_INTAG_3} - ${MYDIAG_INTAG_1}\"" >> ${GNUPLOT_MYDIAG_TMP}
