...
period_of_analysis=['yr'] #,'DJF','MAM','JJA','SON') # 'yr','DJF','MAM','JJA','SON'
#--------------------
...
if sub_plot_flag == 1: 
...
elif sub_plot_flag == 2:
   # SUB AREAS for river analysis
   # North Adriatic, South Adriatic and Aegean, Western Med and Egypt Areas
   Area_ADN_lon=[10,20] # GB LONs 
   ...
   Area_maxlon_ar=[Area_ADN_lon[1],Area_ADE_lon[1],Area_WES_lon[1],Area_EGY_lon[1]]
   Area_minlat_ar=[Area_ADN_lat[0],Area_ADE_lat[0],Area_WES_lat[0],Area_EGY_lat[0]]
   Area_maxlat_ar=[Area_ADN_lat[1],Area_ADE_lat[1],Area_WES_lat[1],Area_EGY_lat[1]]
#
...
if num_of_models == 1:
   if dataset_name == '8':
      ...
      model_path=workdir_path
      model_fileprename='map'
      model_postname='mod_'+dataset_name #'mod_Tides8' or 'mod_Control_run'
      model_label=dataset_name #'Tides_8 run' or 'Control run'
   ...
   model_postname=['mod_rnf2','mod_rnf0'] #'mod_Tides8','mod_Control_run'
   model_label=['rnf2','rnf0'] # 'Tides_8 run','Control run'
   # map3D_yr_%LEV%_%FIELD%_%ANA_STARTDATE%_%ANA_ENDDATE%_mod_%INDATASET%.nc
            ...
            for idx_dt in range(0,len(period_of_analysis)):
                ...
                if Tdiff_threshold_3d == 0:
                   if var_3d == 'vosaline':
                      ...
                      #vals_thresc=0.3
                   #elif str(vlev) == '1' or str(vlev) == '8'or str(vlev) == '30':
                ...
                # --------------------------------------------------
                # Plot the 3 map in different Med areas for currents
                if sub_plot_flag != 0 and grid != 'uv2t':
                #if grid == 'U' or grid == 'V' or grid == 'T':
